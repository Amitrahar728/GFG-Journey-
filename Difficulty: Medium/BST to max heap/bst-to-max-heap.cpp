...
class Solution {
  public:
    ...
    void postorderFill(Node* root, vector<int> &inorderData, int &index) {
        if (root == NULL)
            return;

        postorderFill(root->left, inorderData, index);
        postorderFill(root->right, inorderData, index);
    ...
    void convertToMaxHeapUtil(Node* root) {
        ...
        int index = 0;
        postorderFill(root, inorderData, index); 
    }
};
